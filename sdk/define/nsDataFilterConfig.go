...
type DataFilterConfig struct{
	Name             string                   `json:"name"`          // Unique name for identifying a filter ， eg. export-iotmqtt
	Version          string                   `json:"version"`       // dev 或 res 有修改后，更新索引表，取时间串作为版本号， 发数据时带版本号使用指定索引表、同步时验证版本号是否相同

	Enable           string                   `json:"enable"`       // 版本号变动时置0，同步确认后置1，约定置1前使用原索引关系，置1后使用新索引关系
	//SyncTime         string                   `json:"suyctime"`      // 同步标识，定时每30s同步一次索引，以同步时记录的时间戳为标识 另加通知驱动同步流程
	//Service        DeviceService            `json:"service"`       // Associated Device Service - One per device
	DevRes           []DataFilterByDevRes     `json:"index"`         /// Index 数组，所有dev和res组成的数组，索引取两个数值，devindex和resindex
}

type DataFilterByDevRes struct{
	...
	//Service        DeviceService            `json:"service"`       // Associated Device Service - One per device
	Res              []string                 `json:"resource"`      /// 参数列表，使用数组，取下标作为index使用
}

/**
///========= 获取Appprofile后，初始化DataIndexMap，然后从filterSvr获取索引表详细内容 =========
func (sdk *NSPlusSdk)InitDataIndexMap(profileName string, version string){

	sdk.DataIndexMap = DataIndexForFilter{}
	sdk.DataIndexMap.Name = profileName
...
func (sdk *NSPlusSdk)UpdateDataIndexMap() error {
	...
	return nil
	/****
	url := fmt.Sprintf("%s://%s:%v", Protocol_HTTP, UserConfig.MQTTServer.Host, PORT_FILTERMicSvr)
	res, err := sdk.HttpClient.Get(url + Api_Get_IndexMap + sdk.Profile.Uuid)
	if err != nil {
