...
type  BackDbConfig struct {
	//轮询周期，单位S  因为会存在多个方法添加删除设备数据
	InitDevicePeriod  int64
	// 历史数据库清理周期，单位S
	RealDbClearPeriod int64
	//实时库保留周期，时间单位 天  -1 不清楚
	RealdbSaveTime int
	// 历史通道周期 ，消息队列长度
	HisDbChanLen int
	//非设置默认历史周期 -1
	DefaultRecordTime int64
	//是否持久化数据
	PersistData  bool
	PublicAlarm bool
	PublicReadValue bool
}