...
func LoadFromFile(profile string, configuration interface{}) error {
	msg := fmt.Sprintf(" load configuration profile %s", profile)
	//LoggingClient.Error(msg)
	fmt.Fprintln(os.Stdout, msg)

	path := determinePath()
	fileName := path + "/" + "configuration.toml" //default profile

	msg = fmt.Sprintf(" load configuration filename (%s):path %s:profile %s", fileName, path, profile)
	//LoggingClient.Error(msg)
	fmt.Fprintln(os.Stdout, msg)

	if len(profile) > 0 {
		fileName = path + "/" + profile + "/" + "configuration.toml"
	}

	msg = fmt.Sprintf(" load configuration 3 filename (%s):path %s:profile %s", fileName, path, profile)
	//LoggingClient.Error(msg)
	fmt.Fprintln(os.Stdout, msg)
